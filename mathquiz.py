...
class MathQuizApp:  
    def __init__(self, master):  
        ...
        self.easy_button = tk.Radiobutton(master, text="简单", variable=self.difficulty, value="简单", font=("Arial", 14))  
        self.medium_button = tk.Radiobutton(master, text="中等", variable=self.difficulty, value="中等", font=("Arial", 14))  
        self.hard_button = tk.Radiobutton(master, text="困难", variable=self.difficulty, value="困难", font=("Arial", 14))  
        ...
        # 提交按钮  
        self.submit_button = tk.Button(master, text="提交答案", command=self.check_answer, font=("Arial", 24))  
    ...
    def check_answer(self):  
        ...
        try:  
            ...
            if user_answer == correct_answer:  
                ...
                messagebox.showinfo("正确", f"回答正确！当前得分: {self.score}，连击数: {self.current_streak}")  
            else:  
                messagebox.showerror("错误", f"回答错误！正确答案是: {correct_answer} 连击数重置。")  
                self.current_streak = 0  # 连击数重置到0  

            self.answer_entry.delete(0, tk.END)  # 清空输入框  
            self.generate_question()  # 生成下一个题目  
            self.start_timer()  # 重启计时  
        except ValueError:  
